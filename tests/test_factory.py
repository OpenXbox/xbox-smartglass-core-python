...
@pytest.mark.skip(reason="Broken.. but its deprecated anyways, so?")
def test_message_fragment(packets, crypto):
    msg = factory.message_fragment(
        msg_type=enum.MessageType.Json,
        sequence_begin=12,
        sequence_end=999,
        data='HELLO_ITS_DATA'
    )
    ...
    assert msg.protected_payload.sequence_begin == 12
    assert msg.protected_payload.sequence_end == 999
    assert msg.protected_payload.data == 'HELLO_ITS_DATA'

    assert isinstance(packed, bytes) is True
    assert len(packed) == 26

    assert packed == unhexlify(b'')
